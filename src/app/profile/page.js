        ...
        <div className="glass p-5 rounded-2xl border border-white/10 grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className="mt-1 w-full h-10 glass rounded-xl px-3 text-sm text-white/90 bg-transparent border border-white/15 focus:outline-none focus:ring-2 focus:ring-[rgba(var(--accent-rgb)/0.45)]"
            />
          </div>
          <div>
            <label className="text-[10px] uppercase tracking-wide text-white/50">
          ...
          <div>
            ...
            >
              {["Mild", "Medium", "Hot"].map((s) => (
                <option key={s} className="bg-[var(--background-alt)]">
                  {s}
                </option>
              ))}
            </select>
          ...
          <div className="sm:col-span-2 flex gap-3">
            {!editing ? (
              <button
                ...
                className="text-xs px-3 py-2 rounded-md border border-white/15 hover:border-[rgba(var(--accent-rgb)/0.5)]"
              >
                Edit
              </button>
            ) : (
              <>
                <button
                  onClick={() => {
                    ...
                    updatePreferences({ spice, servings });
                    setEditing(false);
                  }}
                  className="text-xs px-3 py-2 rounded-md border border-[rgba(var(--accent-rgb)/0.5)] bg-[rgba(var(--accent-rgb)/0.15)]"
                >
                  Save
                </button>
                <button
                  onClick={() => {
                    setName(user.name);
                    setEmail(user.email);
                    setSpice(preferences.spice);
                    setServings(preferences.servings);
                    setEditing(false);
                  }}
                  className="text-xs px-3 py-2 rounded-md border border-white/15"
                >
                  Cancel
                </button>
              </>
